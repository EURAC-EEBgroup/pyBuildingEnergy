...
class Building_archetype:
    ...
    def __init__(self, archetype: str, period: str, latitude: float, longitude: float):
        ...
        if archetype in Building_archetype.bui_archetype:
            ...
            raise ValueError(
                f"Invalid choice for archetype. Possible choices are: {', '.join(Building_archetype.bui_archetype)}"
            )
        ...
        if period in Building_archetype.periods:
            ...
            raise ValueError(
                f"Invalid choice for possible periods. Possible choices are: {', '.join(Building_archetype.periods)}"
            )
    ...
    def get_archetype(self, **kwargs):
        ...
        inputs_bui_archetype = list(
            filter(
                lambda x: x.get("type") == self.archetype
                and x.get("year") == self.built_year,
                bui_archetype,
            )
        )
