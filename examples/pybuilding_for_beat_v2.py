...
def process_building(building_archetype, output_dir="results"):
    ...
    try:
        ...
        building_id = building_archetype.get("id", "unknown")
        hourly_file = os.path.join(output_dir, f"hourly_sim_{building_id}.csv")
        annual_file = os.path.join(output_dir, f"annual_results_{building_id}.csv")
...
def main():
    ...
    building_archetypes = list(collection.find({}, {"_id": 0}))

    if not building_archetypes:
        print("No building archetypes found in the database.")
        return
    ...
    print(
        f"Processing {len(building_archetypes)} buildings using {num_processes} processes..."
    )
    ...
    with Pool(processes=num_processes) as pool:
        ...
        results = list(
            tqdm(
                pool.imap(
                    partial(process_building, output_dir=output_dir),
                    building_archetypes,
                ),
                total=len(building_archetypes),
            )
        )
    ...
    print(f"\nSummary:")
    print(f"Total buildings processed: {len(building_archetypes)}")
